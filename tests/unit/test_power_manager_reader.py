...
from powerdeck_backends.system.power_manager import (
    PowerManagerReader,
)
...
def _busctl_key(property_name: str) -> tuple[str, ...]:
    return (
        "busctl",
        "--system",
        "get-property",
...
def test_detects_power_profiles_daemon_and_profiles() -> None:
    ...
    assert (
        state.services[0].activity
        is ServiceActivity.ACTIVE
    )
    assert (
        state.services[1].activity
        is ServiceActivity.INACTIVE
    )
    assert state.services[0].details is None


def test_falls_back_to_system_bus_properties() -> None:
    responses = {
        _systemctl_key(
            "power-profiles-daemon.service"
        ): (
        ...
        _busctl_key("ActiveProfile"): (
            0,
            's "balanced"\n',
            "",
        ),
        _busctl_key("Profiles"): (
            0,
            (
                'aa{sv} 3 1 "Profile" s "power-saver" '
                '1 "Profile" s "balanced" '
                '1 "Profile" s "performance"\n'
            ),
            "",
        ),
    }
...
def test_profile_query_failure_is_recorded() -> None:
    responses = {
        _systemctl_key(
            "power-profiles-daemon.service"
        ): (
    ...
    assert state.available_profiles == ()
    assert state.services[0].details is not None
    assert (
        "powerprofilesctl get"
        in state.services[0].details
    )
    assert (
        "busctl Profiles"
        in state.services[0].details
...
def test_not_found_service_is_reported() -> None:
    ...
    assert (
        service.activity
        is ServiceActivity.NOT_INSTALLED
    )
